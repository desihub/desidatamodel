...
.. toctree::
   ...
   {target}_{reg}_clustering.dat.fits : Main catalog for clustering statistics, including weights. <target> = LRG, ELG, QSO, BGS_ANY, BGS_BRIGHT, ELG_HIP (ELG_HIPnotqso), LRG_main. <reg> = N, S  . <clustering>
   ...
   {target}_{reg}_{RANNUM}_clustering.ran.fits : Randoms associated to a clustering catalog, for given target and region. <target> = LRG, ELG, QSO, BGS_ANY, BGS_BRIGHT, ELG_HIP (ELG_HIPnotqso), LRG_main. <reg> = N, S . <RANNUM> is random number. <clustering_random>
   ...
   {target}_{reg}_nz.txt : N(z) distribution.  <target> = LRG, ELG, QSO, BGS_ANY, BGS_BRIGHT, ELG_HIP (ELG_HIPnotqso), LRG_main. <reg> = N, S  <nz>
   plots/index
   xi/index
