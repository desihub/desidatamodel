====
dark
====

The ``dark`` directory hosts MTL ledgers for the DESI dark-time program.
Targets are stored in ledgers that are split by HEALPixel (in the NESTED
scheme) at a resolution of nside=32.

The filename for each ledger resembles sv2mtl-dark-hp-HPX.ecsv, where HPX
is the healpixel number.

.. toctree::
   :maxdepth: 1

   sv2mtl-dark-hp-HPX.ecsv : MTL ledgers for the DESI dark-time program <sv2mtl-dark-hp-HPX>
