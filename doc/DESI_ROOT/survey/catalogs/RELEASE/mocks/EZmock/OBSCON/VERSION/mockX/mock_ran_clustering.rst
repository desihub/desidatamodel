...
:Naming Convention: ``{TARGET}_{GALCAP}_{RANN}_clustering.ran.fits``, where ``{TARGET}`` is the target: ``QSO``, ``ELG_LOP``, ``LRG`` or ``LRG+ELG_LOP``
                    for dark or ``BGS`` for bright. ``{GALCAP}`` is the Galactic hemisphere region ``NGC`` or ``SGC``. ``{RANN}`` is the number for the random file (18 total, numbered 0 through 17). Each are random with respect to each other.
