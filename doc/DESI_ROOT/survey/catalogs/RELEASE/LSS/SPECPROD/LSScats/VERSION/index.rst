...
Large-scale clustering measurements can be accurately obtained via the columns :py:attr:`RA`, :py:attr:`DEC`, :py:attr:`Z`, :py:attr:`WEIGHT`. Samples are split by Galactic hemisphere, defined by PHOTSYS ``NGC`` and ``SGC``.
...
.. toctree::
   ...
   {TARGET}_{PHOTSYS}_nz.txt : Target densities as a function of redshift <nz_lss>
   {TARGET}_{PHOTSYS}_{RANN}_clustering.ran.fits : Use for clustering measurements (randoms) <random_clustering>
   {TARGET}_{PHOTSYS}_clustering.dat.fits : Use for clustering measurements (data) <data_clustering>
